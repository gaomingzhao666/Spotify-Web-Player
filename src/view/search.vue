<template>
	<div class="search">
		<section class="empty" v-if="data.isEmpty">
...
watch(
	[() => route.query.keywords, () => data.categoryIndex],
	async ([newVal]) => {
		// 跳转前判断，防抖
		...
		if (newVal)
			switch (data.categoryIndex) {
				case 0:
					await init(newVal, 1)
					if (route.name != 'searchSingle')
						router.push({ name: 'searchSingle' })
					break
				case 1:
					await init(newVal, 100)
					if (route.name != 'searchArtist')
						router.push({ name: 'searchArtist' })
					break
				case 2:
					await init(newVal, 1000)
					if (route.name != 'searchPlaylist')
						router.push({ name: 'searchPlaylist' })
					break
				case 3:
